...
def _month_bounds(value: str):
    ...
    return parsed.replace(day=1), parsed.replace(day=monthrange(parsed.year, parsed.month)[1])


def resolve_record(db, selector: RecordSelector):
    query = db.query(BudgetRecord).join(BaseRecord).options(joinedload(BudgetRecord.record))
    ...
    if selector.target == TargetRecord.CATEGORY and selector.category:
        return query.filter(BudgetRecord.category == selector.category).order_by(BaseRecord.created_at.desc()).first()
    return None


def resolve_records(db, categories: list[str] | None = None, filters: dict[str, str] | None = None):
    query = db.query(BudgetRecord).join(BaseRecord).options(joinedload(BudgetRecord.record))
    if categories:
